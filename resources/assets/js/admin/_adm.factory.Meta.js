(function (window, angular) {
	...
	function Meta($ajax, $q, $compile, $rootScope, $timeout, gScope) {
		...
		function Meta($scope) {
            ...
            obj.url_consultar 							= urlhost + '/admin/funcionalidades/meta/get';
			obj.url_gravar 								= urlhost + '/admin/funcionalidades/meta/post';
			obj.url_excluir 							= urlhost + '/admin/funcionalidades/meta/delete';
		...
		function gravar() {
			...
			if (that.SELECTED.VALOR <= 0 || that.SELECTED.VALOR == null || that.SELECTED.VALOR == '') {
				check = false;
				showErro("Valor obrigatório");
			}
        ...
        function compileDatatable(){
			...
			if(obj.DATATABLE == null){
				obj.DATATABLE = $('#dataTableMeta').DataTable({
					"order": [[0, 'desc']],
					"searching": false,
					"data": obj.DADOS,
					"deferRender": true,
					...
					"columns": [
						{ "data": "TIPO", "title": '', "className": "text-center", 
							render : function(data, type, row) {
								var html = '';

								if(data == 0){
									html = '<div class="badge badge-green mr-3 text-small">SALDO EM CAIXA</div>';
								}else if(data == 1){
									html = '<div class="badge badge-danger mr-3 text-small">MARGEM DE SEGURANÇA</div>';
								}
						...
						{ "data": "DESC_META", "title": 'Descrição', 
							render : function(data, type, row) {
								return row.DESC_META;
							} 
						},
						...
						{ "data": "ACTIONS", "title": 'Opções', 'className': 'text-center',
							render : function(data, type, full, meta ) {
								...
								html = html + `
								<button type="button" class="btn btn-sm btn-danger" ng-click="vm.Meta.excluir(`+index+`);" ng-dblclick="$event.stopPropagation();">
