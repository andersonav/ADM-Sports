    ...
    public function getFluxoCaixaDiario(){
        ...
        $sqlSaldo = "SELECT 
                    ...
                    FROM (
                        SELECT DISTINCT
                            T.DATA,
                            COALESCE((SELECT SUM(X.VALOR_TOTAL) X FROM TBLANCAMENTO X INNER JOIN TBMODULO_CONTA_TIPO TP ON TP.ID = X.MODULO_CONTA_TIPO_ID WHERE TP.OPERACAO = 0), 0) AS ENTRADAS,
                            
                            COALESCE((SELECT SUM(X.VALOR_TOTAL) X FROM TBLANCAMENTO X INNER JOIN TBMODULO_CONTA_TIPO TP ON TP.ID = X.MODULO_CONTA_TIPO_ID WHERE TP.OPERACAO = 1), 0) AS SAIDAS
                        FROM TBLANCAMENTO T
                        WHERE 
